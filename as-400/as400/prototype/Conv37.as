/**
 * @file Conv37.java
 *
 * Converts characters between CCSID 37 and ASCII.
 */

package as400.prototype {

    import flash.utils.*;

    public class Conv37 {

        /**
         * Convert an EBCDIC character string to a UTF-8 string.
         */
        public static function from_ebcdic(ebcdic_str:ByteArray, offset:int = 0) : String {

            ebcdic_str.position = offset;
            var r:String = "";

            var len:uint = ebcdic_str.length - offset;
            for(var i:uint = 0; i < len; i++) {

                var ch:int = ebcdic_str.readByte();
                if(ch < 0)
                    ch &= 0xFF; // ch = 256 + ch;
                r += String.fromCharCode(_qascii[ch]);
            }

            ebcdic_str.position = 0;
            return r;
        }

        /**
         * Convert a String a to an EBCDIC string.
         */
        public static function to_ebcdic(pc_str:String) : ByteArray {

            var r:ByteArray = new ByteArray();

            var len:uint = pc_str.length;
            for(var i:uint = 0; i < len; i++) {

                var ch:int = int(pc_str.charCodeAt(i));
                if(ch < 0)
                    ch = 256 + ch;
                r.writeByte(_qebcdic[ch]);
            }

            r.position = 0;
            return r;
        }

        /// Content of *TBL QSYS/QASCII
        public static var _qascii:Vector.<int>
            = new <int>[
                        
                        ];

        /// Content of *TBL QSYS/QEBCDIC
        public static var _qebcdic:Vector.<int>
            = new <int>[
                        
                        ];

    } // class

} // package
